...
class TrainModel(object):
    ...
    def __init__(
        ...
        teacher_model = models.regnet_y_16gf(pretrained=True)
        teacher_model.fc = nn.Linear(3024, self.model.num_classes)
        
        self.loss_func = DistillationLoss(
    ...
    @torch.no_grad()
    def validate_on_batch(self, validation_data, epoch, log_step):
        self.model.eval()
        ...
        for batch, (images, labels) in enumerate(validation_data):
            ...
            if (batch+1) % log_step == 0:
                print(f'\n[Batch {batch+1}/{len(validation_data)}]'
                      f'   valid loss: {loss:.3f} acc: {acc:.3f}')
